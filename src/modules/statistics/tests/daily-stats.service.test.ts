...
describe('DailyStatsService', () => {
  ...
  describe('getStats', () => {
    it('should return paginated stats with default values', async () => {
      ...
      mockStatsRepository.findAndCount = jest.fn().mockResolvedValue([mockStats, 1])

      const result = await service.getStats()

      expect(mockStatsRepository.findAndCount).toHaveBeenCalledWith({
    ...
    it('should return paginated stats with custom offset and limit', async () => {
      ...
      mockStatsRepository.findAndCount = jest.fn().mockResolvedValue([mockStats, 100])

      const result = await service.getStats(20, 50)
    ...
    it('should return empty array when no stats found', async () => {
      mockStatsRepository.findAndCount = jest.fn().mockResolvedValue([[], 0])

      const result = await service.getStats(0, 10)
    ...
    it('should order results by stat_date DESC', async () => {
      ...
      mockStatsRepository.findAndCount = jest.fn().mockResolvedValue([mockStats, 2])
